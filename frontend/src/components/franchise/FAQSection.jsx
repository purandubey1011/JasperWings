...
const FAQSection = () => {
  ...
  const faqs = [
    { question: "What is the minimum investment required?", answer: "The minimum investment ranges between $150k - $250k depending on location size and condition." },
    { question: "Do I need restaurant experience?", answer: "While preferred, it is not mandatory. We provide comprehensive training for all new franchisees." },
    { question: "How long till opening?", answer: "Typically 3-6 months from signing the agreement to opening day." },
    { question: "Will I get supply & training?", answer: "Yes, we provide full supply chain support and a 4-week training program." }
  ];

  return (
    <section className="relative py-24 px-4 md:px-10 bg-black overflow-hidden w-full">
          ...
          {faqs.map((faq, index) => (
            ...
            >
              <button 
                ...
                className="w-full flex justify-between items-center p-6 md:p-8 text-left hover:bg-gray-900 transition-colors duration-200"
              >
                {/* Question Text */}
                ...
                <ChevronDown 
                  size={24}
                  className={`text-gray-400 transform transition-transform duration-300 ${openFaq === index ? 'rotate-180' : ''}`} 
                />
              </button>

              {/* Accordion Content */}
              <div 
                className={`overflow-hidden transition-[max-height] duration-300 ease-in-out ${openFaq === index ? 'max-h-48' : 'max-h-0'}`}
              >
                <div className="p-6 md:p-8 pt-0 text-gray-400 text-base md:text-lg leading-relaxed">
                    {faq.answer}
                </div>
              </div>
            </div>
